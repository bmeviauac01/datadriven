...
The goal of the seminar is to understand the concepts of the _MongoDB_ document database and the usage of the _MongoDB C#/.NET Driver_.
...
1. Start the MongoDB server.
    ...
    - Open a _command prompt_ and start MongoDB server: `mongod.exe --dbpath="c:\work\<NEPTUN>\db"`
        ...
        !!! info ""
            _Command prompt_ can be located in the start men by typing "cmd".
...
1. Create the database.
    ...
    - Open a **new** _command prompt_ and initialize database: `mongo.exe localhost:27017/datadriven c:\work\<NEPTUN>\mongo.js`
        ...
        !!! note ""
            ...
            In the university computer laboratories this executable too is located at `c:\tools\mongodb\bin`.
...
1. Let us examine this project.
    ...
    - `Program.cs` already contains the initialization of the connection to MongoDB.
        ...
        - Interface `IMongoDatabase` represents database `datadriven` within the MongoDB server.
...
Write C# code using the _MongoDB C#/.NET Driver_ in the following exercises. Print the results to console.

1. List the names and the amount of stock of all products that we have more than 30 in stock!

1. List the orders that consist or at least two items!
...
??? example "Solution"
    ...
    1. A simple query is not sufficient for this exercise, thus we need the aggregation pipeline. We may still note that due to the schema every information we need is still available in one collection.
    ...
    1. To find the most expensive product we need two queries: first to find the largest price value, then to find the products with this price.
    ...
    1. This exercise is complicated with our current database scheme because we do not have everything at hand within one collection. We need the product information from one collection, and the order details from another one.
        ...
        We will be doing a "join" in the client-side, that is, in C# code. The outline of the solution is to query the orders, then in C# gather the orders by product, and finally, query the product details.
        ...
        This solution is very elegant and works only for small databases. If we face a similar task under real-life circumstances, we have two choices: denormalize the database scheme and copy product details into the orders, or create an aggregation pipeline executed by the server that does something similar to the code above (MongoDB can do that, but it will not be very fast).

## Exercise 2: Create new entity class

1. Examine classes `Product` and `VAT`. Why is there a field with a `[BsonId]` attribute in class `Product` and not in class `VAT`?
...
??? example "Solution"
    1. Class `Product` represents the `products` collection, therefore each item has a unique `ObjectID`. Class `VAT` on the other hand is an embedded field used by `Product` and has no collection on its own, hence needs no id.
...
??? example "Solution"
    ...
    1. Query categories that contain any product, then delete the ones that do not belong among this list.
        ...
        Let us note that this is not an atomic operation. If a product has been added concurrenctly, we could have deleted its category.
