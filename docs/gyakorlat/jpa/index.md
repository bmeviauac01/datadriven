...
1. Tekintsük át röviden a projektet:
    ...
    - Az _application.properties_-ben van pár alapvető beállítás, itt a DB eléréshez **ellenőrizzük az adatbázis URL-t (spring.datasource.url), a usernevet (spring.datasource.username) és jelszót (spring.datasource.password)**. Klasszikus Java EE alkalmazásban a `persistence.xml`-be írnánk be az adatbázis JNDI nevét, de a Spring Boot XML nélküli konfigurációt is támogat, itt ezt használjuk ki.
